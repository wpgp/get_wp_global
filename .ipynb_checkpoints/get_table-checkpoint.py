...
def get_mask_from_poi(v: gpd.GeoDataFrame, 
    ...
    **kwargs):
    ...
    if 'rad' in kwargs:
        rad = np.round(np.degrees(kwargs['rad']/6378)/d)
    else:
    ...
    ix = np.floor((v.geometry.x.values - bds[0])/d).astype(int)
    iy = np.floor((bds[3] - v.geometry.y.values)/d).astype(int)
    
    arr = np.zeros((sx,sy))
    arr[ix,iy] = v['id'].values
    arr = arr.T
    
    crc = utils.radial_kernel(size, lambda r: np.where(r <= rad, float(1), 0.0))
    msk = utils.max_conv_2d(arr, crc, mode='same', weighted=True)
    m = msk == 0
...
def get_mask(v: gpd.GeoDataFrame, 
    resolution: Optional[str] = '1km',
    ext_vrt: Optional[tuple] = None,
    **kwargs):
...
def extract(gdf: gpd.GeoDataFrame, 
    ...
    **kwargs):
    ...
    msk = get_mask(gdf, ext_vrt=ext_vrt, **kwargs)
    bds = msk['bounds']
    ...
    pop_data = pop.ReadAsArray(xoff=ll[0], yoff=ur[1], 
                               win_xsize=msk['mask'].shape[1], 
                               win_ysize=msk['mask'].shape[0])
    
    #Performing zonal statistics
    #rast = rasterise(gdf, pop_data, nodata=nodata)
    stat = get_raster_stats(pop_data, 
                            msk['mask'],
                            w=msk['weight'], 
                            nodata=nodata,
                            skip=[0], 
    ...
    **kwargs):
    ...
    if (np.all(gdf.geometry.geom_type == 'Point')):
        ...
        if 'weighted' in kwargs:
            mask,weight,bds = get_mask_from_poi(gdf, **kwargs)
        else:
            mask,bds = get_mask_from_poi(gdf, **kwargs)
            weight = None
    else:
        mask = rasterise(gdf, pop_data, nodata=-1)
        weight = None
...
def get_data(gdf: gpd.GeoDataFrame, 
    ...
    **kwargs):
    ...
    result = extract(gdf, vrt_path, resolution=resolution, return_gdf=return_gdf, explode=explode, **kwargs)
    return result
